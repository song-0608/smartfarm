...
export default function Home() {
  ...
  const processVoiceQuery = useCallback(
    (text: string) => {
      ...
      let reply = "";
      if (text.includes("天气") || text.includes("温度") || text.includes("下雨")) {
        reply = plantingWeather
          ? `当前天气：${plantingWeather.description}，温度${plantingWeather.temperature}℃，湿度${plantingWeather.humidity}%，风速${plantingWeather.windSpeed}km/h。平均温度${plantingWeather.avgTemperature}℃，预计降水${plantingWeather.totalPrecipitation}mm。`
          : "暂无天气数据，请先在种植规划中获取GPS天气信息。";
      } else if (text.includes("价格") || text.includes("行情") || text.includes("多少钱")) {
        const top3 = marketData.slice(0, 3);
        reply = top3.length > 0
          ? `今日热门行情：${top3.map((m) => `${m.emoji}${m.name} ${m.price}元/斤 ${m.change || ""}`).join("，")}`
          : "暂无市场数据，请稍后再试。";
      } else if (text.includes("病") || text.includes("虫") || text.includes("害")) {
        reply = "建议您拍摄受病害影响的作物照片，我可以帮您分析诊断。常见病害防治建议：\n1. 保持田间通风透光\n2. 合理施肥增强抗病力\n3. 发现病株及时清除\n4. 必要时使用生物农药";
      } else if (text.includes("种") || text.includes("规划") || text.includes("建议")) {
        reply = "建议您使用种植规划功能，通过AI分析土地和天气条件，获取最适合您地块的作物推荐方案。点击底部导航的种植标签即可开始。";
      } else {
        reply = `我理解您的问题是"${text}"。作为农业AI助手，我可以帮您查天气、问价格、诊断病害、提供种植建议。请尝试更具体的提问。`;
      }
      setChatMessages((prev) => [...prev, { role: "ai", text: reply }]);
    },
    [plantingWeather, marketData]
  );
  ...
  const handleSendMessage = useCallback(() => {
    ...
    setTimeout(() => {
      let reply = "";
      if (text.includes("天气") || text.includes("温度")) {
        reply = plantingWeather
          ? `当前天气：${plantingWeather.description}，温度${plantingWeather.temperature}℃，湿度${plantingWeather.humidity}%。`
          : "暂无天气数据，请先获取GPS天气信息。";
      } else if (text.includes("价格") || text.includes("行情")) {
        const top3 = marketData.slice(0, 3);
        reply = top3.length > 0
          ? `今日行情：${top3.map((m) => `${m.emoji}${m.name} ${m.price}元/斤`).join("，")}`
          : "暂无市场数据。";
      } else {
        reply = "感谢您的提问！我正在学习更多农业知识，建议您尝试询问天气、价格、病害诊断或种植建议相关问题。";
      }
      setChatMessages((prev) => [...prev, { role: "ai", text: reply }]);
    }, 800);
  }, [inputText, plantingWeather, marketData]);
